...
(function ($) {
  ...
  var barcode = {
    ...
    // clear jQuery Target
    // [MMA] adds .css("height", "55px") & replace .css("width", w + "px") by .css("width", "300px")
    ...
    digitToBmpRenderer: function($container, settings, digit, hri, mw, mh){
      ...
      object.setAttribute('style', 'margin-left:-10px;max-width:310px;');
      object.setAttribute('type', 'image/bmp');
      object.setAttribute('data', 'data:image/bmp;base64,'+ this.base64Encode(bmp));
      // [MMA] remove this.resize($container, mw * columns + padding).append(object);
      this.resize($container, 300).append(object);
  ...
  $.fn.extend({
    barcode: function(datas, type, settings) {
      ...
      if (typeof(datas) == "string"){
          // [MMA] adds this condition to remove Order if is present in datas barcode is shorter for the receipt ;)
          if(datas.includes("Order")){
              code = datas.replace("Order ", "");
          }else{
              code = datas;
          }
