...
class ProductTemplate(models.Model):
    ...
    product_product_ids = fields.One2many("price.history", 'product_ids', string='Sales Price History')
...
class SaleOrderLine(models.Model):
    ...
    product_ids = fields.Many2one('product.template', string='Product', domain=[('sale_ok', '=', True)], ondelete='restrict')
...
#             product._cr.execute("delete from price_history where product_id =  %s" %(product.id))
            sale_order_line_obj = self.env['sale.order.line'].search([('product_ids', '=', product.id)])
            ids=[]
            price_history_obj = self.env['price.history']
            for sales in sale_order_line_obj:
                if sales.product_ids == product and product.company_id.price_history == 'confirm_order' and sales.order_id.state =='sale' and int <= product.company_id.item_limit:
                    products_dic = {
                    ...
                    int += 1
                elif sales.product_ids == product and product.company_id.price_history == 'done' and sales.order_id.state =='done' and int <= product.company_id.item_limit:
                    products_dic = {
                                'customer' : sales.order_id.partner_id.id,
                                'order_date' : sales.order_id.date_order,
                                'product_ids' : product.id,
                    ...
                    ids.append(price_history_rec.id)
                    int += 1
                elif sales.product_ids == product and product.company_id.price_history == 'both' and sales.order_id.state in ('sale','done') and int <= product.company_id.item_limit:
                    products_dic = {
                                'customer' : sales.order_id.partner_id.id,
                                'order_date' : sales.order_id.date_order,
                                'product_ids' : product.id,
