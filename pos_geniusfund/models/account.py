...
class AccountTax(models.Model):
    ...
    def compute_all(self, price_unit, currency=None, quantity=1.0,
        ...
        # Override to calculate excise taxs
        ...
        for tax in self.sorted(key=lambda r: r.sequence):
            ...
            if tax.amount_type == 'group':
                ...
                ret = children.compute_all(price_unit, currency, quantity,
                                           product, partner)
                ...
                if 'excise' in tax.name.lower():
                    tax_amount = tax._compute_amount(
                        (product.lst_price * quantity),
                        (product.lst_price * quantity), quantity, product,
                        partner)
                    print("::::::tax_amount::::", tax_amount)
                else:
                    tax_amount = tax._compute_amount(base, price_unit,
                                                     quantity, product,
                                                     partner)
