...
class PosOrderLine(models.Model):
    """ Defines THC quantity calculation """
    _inherit = 'pos.order.line'

    x_studio_total_weight = fields.Float(string='Total Weight', readonly=True, store=True, default=0,
                                         compute='_eval_total_weight')

    @api.multi
    @api.depends('x_studio_weight_per_unit', 'qty')
    def _eval_total_weight(self):
        """ Evaluate global THC consumption on every order line.
            The global "THC value" cannot be negative when returning products
         """
        for record in self:
            if record.x_studio_weight_per_unit and record.qty > 0:
                record.x_studio_total_weight = record.x_studio_weight_per_unit * record.qty
            else:
                record.x_studio_total_weight = 0

class pos_config(models.Model):
    _inherit = "pos.config"

    enable_cash_limit = fields.Boolean("Enable Cash Limit")
    set_limit = fields.Integer("Set Cash Limit", default=2500)

    @api.model
    def load_config(self, config_id):
        if config_id:
            config_obj = self.search_read(
                [('id', '=', config_id)], ['enable_cash_limit','set_limit'])
            if config_obj:
                return config_obj
        return False
...
class AccountBankStatement(models.Model):
    ...
    @api.model
    def get_total_cash(self, ongoing_transaction_amt, session_id):
        statement_ids = self.search([('pos_session_id', '=', session_id)])
        cash_amt = 0
        for stmt_id in statement_ids:
            if stmt_id.journal_id.type == 'cash':
                cash_amt += abs(stmt_id.difference)
        return cash_amt + ongoing_transaction_amt
...
class PosOrder(models.Model):
    ...
    def _prepare_account_move_and_lines(self, session=None, move=None):
        # Override to calculate excise tax
        ...
        for order in self.filtered(lambda o: not o.account_move or o.state == 'paid'):
            ...
            for line in order.lines:
                ...
                for tax in taxes.compute_all(price, cur, line.qty, line.product_id)['taxes']:
                    if cur != cur_company:
